...
parser.add_option("-n", "--num_rois", type="int", dest="num_rois", help="Number of ROIs per iteration. Higher means more memory use.", default=15)
parser.add_option("--config_filename", dest="config_filename", help="Location to read the metadata related to the training (generated when training).", default="training_configs/frcnn.pickle")
...
all_imgs, classes_count, class_mapping_2 = get_data(options.test_path, test_only=True)
...
with open('log/frcnn/frcnn_results.csv', 'w') as result_csv:
	...
	log_writer.writerow(['iou_threshold', 'precision', 'recall', 'f1_score', 'avg_precision', 'mean_distance',
...
for iou_threshold_tp in [0.1, 0.2, 0.3, 0.4]:
	...
	for img_data in all_imgs:
		...
		for jk in range(R.shape[0]//C.num_rois + 1):
			...
			for ii in range(P_cls.shape[1]):
				# check whether classifier is too unsure (<0.8) or detected the background class
				...
				bboxes[cls_name].append([16 * x, 16 * y, 16 * (x + w + 1), 16 * (y + h + 1)])
		...
		for key in bboxes:
			...
			if options.write and iou_threshold_tp == 0.1:
				...
				if not os.path.isdir("results"):
					os.mkdir("results")
				cv2.imwrite(f'./results/frcnn/{img_data["filepath"].split("/")[-1]}', img_cropped)
	...
	with open('log/frcnn/frcnn_results.csv', 'a') as result_csv:
		log_writer = csv.writer(result_csv, delimiter=';')
		log_writer.writerow([iou_threshold_tp, precision, recall, f1_score, avg_precision, mean_distance,
							 precision_b_small, precision_b_medium, precision_b_large,
							 recall_b_small, recall_b_medium, recall_b_large,
							 mean_distance_b_small, mean_distance_b_medium, mean_distance_b_large])

	with open('log/frcnn/frcnn_height_fn_' + str(round(iou_threshold_tp, 1)) + '.csv', 'w') as height_csv:
