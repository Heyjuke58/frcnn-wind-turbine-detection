...
from keras_frcnn.simple_parser import get_data_modified
...
parser.add_option("--config_filename", dest="config_filename", help="Location to store all the metadata related to the training (to be used when testing).", default="training_configs/rpn_modified.pickle")
...
all_imgs, classes_count, class_mapping = get_data_modified(options.train_path)
...
model_rpn.compile(optimizer=optimizer, loss=[loss_func.rpn_modified_loss_cls(), loss_func.rpn_modified_loss_coord()])
...
with open('log/rpn_modified/rpn_modified_training.csv', 'w') as log:
	log_writer = csv.writer(log, delimiter=';')
	log_writer.writerow(['curr_loss', 'loss_rpn_cls', 'loss_rpn_coord', 'curr_val_loss', 'val_loss_rpn_cls', 'val_loss_rpn_regr'])
for epoch_num in range(num_epochs):
	progbar = generic_utils.Progbar(epoch_length)
	print('Epoch {}/{}'.format(epoch_num + 1, num_epochs))

	# first 3 epoch is warmup
	# if epoch_num == 3 and options.load is not None:
	...
	while True:
		...
		if iter_num == epoch_length:
			...
			with open('log/rpn_modified/rpn_modified_training.csv', 'a') as log:
				log_writer = csv.writer(log, delimiter=';')
			...
			if curr_loss < best_loss:
				...
				best_loss = curr_loss
				model_rpn.save_weights(f'./models/rpn_modified/rpn_modified.{options.network}.weights.{epoch_num}-{round(curr_loss, 2)}-{round(curr_val_loss, 2)}.hdf5')
